...
class TaskStatusView(APIView):
    ...
    def get(self, request):
        total_projects = Site.objects.all().count()
        projects_open = Site.objects.filter(final_acceptance_cert_comment__isnull=True).count()
...
class TimesheetSummaryView(APIView):
    ...
    def get(self, request):
        ...
        total_fa = Site.objects.filter(final_acceptance_cert_comment__isnull=False).count()
